...
class SearchForm(forms.Form):
    generic_text = forms.CharField(
        label="", strip=True, widget=forms.TextInput(attrs={"placeholder": "Search..."}), required=False
    )
...
class AdvancedSearchForm(forms.Form):
    ...
    text_search_fields = forms.MultipleChoiceField(
        label="Search Card:", choices=CONS.TEXT_SEARCH_FIELD_CHOICES, required=False
    )
    ...
    colour_combination = forms.ChoiceField(
        label="Color combinations:", choices=CONS.DATABASE_COLOUR_COMBINATION_CHOICES, required=False
    )
    ...
    card_type = forms.MultipleChoiceField(
        label="Card Type(s):", choices=CONS.DATABASE_CARD_TYPE_CHOICES, required=False
    )
    ...
    atk_value = forms.IntegerField(
        label="ATK", required=False, min_value=0, widget=forms.NumberInput(attrs={"placeholder": "Attack"})
    )
    atk_comparator = forms.ChoiceField(required=False, choices=CONS.ATK_DEF_COMPARATOR_CHOICES)
    def_value = forms.IntegerField(
        label="DEF", required=False, min_value=0, widget=forms.NumberInput(attrs={"placeholder": "Defense"})
    )
...
class AddCardForm(forms.ModelForm):
    ...
    ability_texts = forms.CharField(
        ...
        widget=forms.Textarea(
            attrs={
                "placeholder": "Each ability must be separated by TWO newlines at the end. Single newlines will be counted as part of 1 ability."
            }
        ),
    )
    ...
    class Meta:
        ...
        widgets = {
            ...
            "cost": forms.TextInput(
                attrs={
                    "placeholder": 'Each value must be inside {}, GRUWB, T for Time, M for Moon e.g. "{1}{M}{T}{W}{W}", "0" if "0" and blank if no cost '
                }
            ),
    ...
    def __init__(self, *args, **kwargs):
        ...
        for visible in self.visible_fields():
            if (
                isinstance(visible.field.widget, forms.widgets.CheckboxSelectMultiple)
...
class UserLoginForm(AuthenticationForm):
    ...
    username = UsernameField(
        widget=forms.TextInput(attrs={"class": "form-control", "placeholder": "Username", "id": "username-field"})
    )

    password = forms.CharField(
        widget=forms.PasswordInput(attrs={"class": "form-control", "placeholder": "Password", "id": "password-form"})
    )
