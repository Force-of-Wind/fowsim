...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for period in pick_periods:
            if period.all_time:
                cards = (
                    Card.objects.filter(decklistcard__isnull=False)
                    .annotate(number_of_decks=Count("decklistcard"))
                    .order_by("-number_of_decks")
                )
                ...
                cards = (
                    cards.annotate(
                        number_of_decks=Count(
                            "decklistcard__decklist__id",
                            filter=Q(decklistcard__decklist__last_modified__gte=period_dt),
                            distinct=True,
                        )
                    )
                    .filter(number_of_decks__gte=1)
                    .order_by("-number_of_decks")
                )

            deck_counts = decklists.count()
            for card in cards[:30]:
                MostPickedCardPickRate.objects.create(
                    card=card, percentage=int(card.number_of_decks * 100 / deck_counts), period=period
                )
            ...
            if total_cards:
                for card_attr in CardColour.objects.all():
                    ...
                    AttributePickRate.objects.create(
                        period=period, card_attr=card_attr, percentage=int(attr_count * 100 / total_cards)
                    )
                ...
                for card_type in Type.objects.all():
                    ...
                    CardTypePickRate.objects.create(
                        period=period, card_type=card_type, percentage=int(type_count * 100 / total_cards)
                    )
                ...
                for i in range(0, 17):
                    CardTotalCostPickRate.objects.create(
                        period=period, percentage=int(total_costs[i] * 100 / total_cards), total_cost=i
                    )
