"""
Tests for the migrate_modal_cards management command.
"""
...
class TestMigrateModalCards:
    def _modal_pair(self, card_colour, card_type):
    ...
    def _transform_pair(self, card_colour, card_type):
        """A genuine (non-modal) two-sided/transform pair sharing the ^ pool."""
    ...
    def test_marks_only_named_pair(self, card_colour, card_type):
        top, bottom = self._modal_pair(card_colour, card_type)
        front, back = self._transform_pair(card_colour, card_type)

        out = StringIO()
        call_command("migrate_modal_cards", "--modal-ids", "XXX-064", "--commit", stdout=out)
        ...
        assert front.modal_face is None
        assert back.modal_face is None
    ...
    def test_no_op_without_commit(self, card_colour, card_type):
        ...
        out = StringIO()
        call_command("migrate_modal_cards", "--modal-ids", "XXX-064", stdout=out)

        top.refresh_from_db()
        bottom.refresh_from_db()
        assert top.modal_face is None
        assert bottom.modal_face is None
    ...
    def test_refuses_j_ruler_partner(self, card_colours, card_types):
        ...
        out = StringIO()
        call_command("migrate_modal_cards", "--modal-ids", "ZZZ-100", "--commit", stdout=out)

        ruler.refresh_from_db()
        assert ruler.modal_face is None
    ...
    def test_backfill_all_recomputes(self, card_colour, card_type, paradoxical_type):
        ...
        out = StringIO()
        call_command("migrate_modal_cards", "--backfill-all", "--commit", stdout=out)
