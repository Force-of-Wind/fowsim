...
def apply_text_search(cards, text, search_fields, exactness_option):
    ...
    if "name" in search_fields:
        # display_name carries the combined "top//bottom" for alternative cards, so searching the
...
def basic_search(basic_form):
    ...
    if basic_form.is_valid():
        ...
        cards = (
            ...
            .exclude(get_alternative_bottom_exclusion_query())
            .distinct()
        )
...
def advanced_search(advanced_form):
    ...
    if advanced_form.is_valid():
        ...
        alternative_query = get_alternative_query(advanced_form.cleaned_data["alternative"])
...
def get_alternative_query(alternative):
    return Q(alternative_face__isnull=False) if alternative else Q()


def get_alternative_bottom_exclusion_query():
    """Alternative bottom halves are never independently addable - exclude them from search results."""
    return Q(alternative_face=Card.ALTERNATIVE_FACE_BOTTOM)


def normalise_alternative_bottom_halves(decklist_cards):
    """
    Legacy decks may reference a alternative bottom half. The top half is the deck representative,
    ...
    for deck_card in decklist_cards:
        if deck_card.card.alternative_face == Card.ALTERNATIVE_FACE_BOTTOM and deck_card.card.alternative_partner is not None:
            deck_card.card = deck_card.card.alternative_partner
